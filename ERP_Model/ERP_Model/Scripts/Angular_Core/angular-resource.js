    ...
    angular.module("ngResource", ["ng"])
        .provider("$resource",
            function() {
                ...
                this.$get = [
                    "$http", "$log", "$q", "$timeout", function($http, $log, $q, $timeout) {
                                ...
                                forEach(url.split(/\W/),
                                    function(param) {
                                        ...
                                        if (!(new RegExp("^\\d+$").test(param)) &&
                                            ...
                                            urlParams[param] = {
                                                isQueryParamValue: (new RegExp("\\?.*=:" + param + "(?:\\W|$)"))
                                                    .test(url)
                                            };
                                        }
                                    });
                                url = url.replace(/\\:/g, ":");
                                url = url.replace(PROTOCOL_AND_DOMAIN_REGEX,
                                    function(match) {
                        ...
                        function resourceFactory(url, paramDefaults, actions, options) {
                            ...
                            forEach(actions,
                                function(action, name) {
                                    ...
                                    var cancellable = angular.isDefined(action.cancellable)
                                        ? action.cancellable
                                        : (options && angular.isDefined(options.cancellable))
                                        ? options.cancellable
                                        ...
                                        if (!isInstanceCall && cancellable) {
                                            ...
                                            if (numericTimeout) {
                                                numericTimeoutPromise =
                                                    $timeout(timeoutDeferred.resolve, numericTimeout);
                                            }
                                        }
