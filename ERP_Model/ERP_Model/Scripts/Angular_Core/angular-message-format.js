...
// Require this to call $compile.$$addBindingInfo() which allows Protractor to find elements by binding.
        return parsedFn;
    }
 ...
 */
    ...
    InterpolationParts.prototype.toParsedFn = function toParsedFn(mustHaveExpression, originalText) {
        ...
        parsedFn["expressions"] = new Array(this.expressionFns
            .length);
// Require this to call $compile.$$addBindingInfo() which allows Protractor to find elements by binding.
        for (var i = 0; i < this.expressionFns.length; i++) {
    ...
    function InterpolationPartsWatcher(interpolationParts, scope, listener, objectEquality) {
        ...
        this.listener = listener;
        var self = this;
        this.expressionFnsWatcher = scope["$watchGroup"](interpolationParts.expressionFns,
            function(newExpressionValues, oldExpressionValues) {
                self.watchListener(newExpressionValues, oldExpressionValues);
            });
    }

    InterpolationPartsWatcher.prototype
        .watchListener = function watchListener(newExpressionValues, oldExpressionValues) {
 ...
 * @private
 */
    function MessageFormatParser(text,
        startIndex,
        $parse,
        pluralCat,
        stringifier,
...
// Run through our grammar avoiding deeply nested function call chains.
    ...
    MessageFormatParser.prototype.rulePluralValueOrKeyword = function rulePluralValueOrKeyword() {
        ...
        if (match == null) {
            this.parsedFn = new PluralMessage(this.expressionFn, this.choices, this.pluralOffset, this.pluralCat)
                .parsedFn;
            this.rule = null;
            return;
        }
...
// startSymbol/endSymbol when I add the feature to allow them to be redefined.
    ...
    MessageFormatParser.prototype
        .ruleEndMustacheInInterpolationOrMessage = function ruleEndMustacheInInterpolationOrMessage() {
            var expressionFn = this.parsedFn;
            this.popState();
            this.interpolationParts.addExpressionFn(expressionFn);
            this.rule = this.ruleInInterpolationOrMessageText;
    ...
    MessageFormatParser.prototype.ruleEndMustache = function ruleEndMustache() {
        ...
        if (this.parsedFn == null) {
            ...
            this.parsedFn["exp"] = this
                .expressionFn["exp"]; // Needed to pretend to be $interpolate for tests copied from interpolateSpec.js
            this.parsedFn["expressions"] = this
 ...
 */
    ...
    var $$MessageFormatFactory = [
        "$parse", "$locale", "$sce", "$exceptionHandler", function $$messageFormat(
            $parse,
            $locale,
            $sce,
            $exceptionHandler) {
            ...
            function interpolate(text, mustHaveExpression, trustedContext, allOrNothing) {
                ...
                var parser = new MessageFormatParser(text,
                    0,
                    $parse,
                    $locale["pluralCat"],
                    stringifier,
    ...
    var $$interpolateDecorator = [
        "$$messageFormat", "$delegate", function $$interpolateDecorator($$messageFormat, $interpolate) {
            if ($interpolate["startSymbol"]() != "{{" || $interpolate["endSymbol"]() != "}}") {
                throw $interpolateMinErr("nochgmustache",
